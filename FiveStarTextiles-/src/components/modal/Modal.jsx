...
const Modal = ({ name, address, pincode, phoneNumber,paymentMethod, setName, setAddress, setPincode, setPhoneNumber, setPaymentMethod, buyNow }) => {
    ...
    function openModal() {
        if(cartItems.length > 0) {
            if(paymentMethod === 'online_payment' || paymentMethod === 'cash_on_delivery')
            {
                setIsOpen(true)
            }
            else{
                                        ...
                                        <div className="flex flex-col items-center justify-center py-8 mx-auto  lg:py-0">
                                            ...
                                            <div className="w-full  rounded-lg md:mt-0 sm:max-w-md xl:p-0 ">
                                                <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                                                    ...
                                                    <button onClick={()=>{buyNow(); closeModal(); navigate('/order')}} type="button" className="focus:outline-none w-full text-white bg-green-600 hover:bg-violet-800  outline-0 font-medium rounded-lg text-sm px-5 py-2.5 ">Order Now</button>
                                                </div>
                                            </div>
                                        </div>
                                    </section>
